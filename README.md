...
```
Copies each product’s **Featured Image** and **original-art** (custom field URL). Skips files when the destination has the **same size**. Background-safe (batches continue server-side) with a live progress pie and a nightly cron window (1–5 AM).

## Auto-Updates
Built-in updater points to **emkowale/buzzsaw**. Publish GitHub Releases with tags like `v1.1.6` and an attached zip `buzzsaw-v1.1.6.zip` (zip of the `buzzsaw/` folder). WordPress will detect and install updates.

## Full Blueprint (Source)
You can rebuild the plugin from this README alone.
...
class Buzzsaw_Pusher {
    const STATE_KEY = 'buzzsaw_push_state'; // transient state
...
## Release
Run `./release major|minor|patch` to bump, tag, build, push, and create a release with the zip.
